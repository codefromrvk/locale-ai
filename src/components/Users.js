...
export  function Users({sameAreaUsers,areasObj}) {
  ...
  function handleInput(e){
    ...
    setChartData([{
      name:e.target.value,
      value:sameAreaUsers[aread_id].userlist.length
    }])
    console.log("set",{
      name:e.target.value,
      value:sameAreaUsers[aread_id].userlist.length
   ...
   { Object.keys(sameAreaUsers).map(area=>{
     return <option>{areasObj[area]}</option>
        ...
        >
          ...
          <YAxis />
          <Tooltip />
          <Legend payload={[{ value: 'Number of users per area ', type: 'square',color:'green'}]}/>
